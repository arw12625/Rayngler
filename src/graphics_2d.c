...
void loadShaderProgramG2D(Graphics2D *g2D, GLchar *vertexsource, GLchar *fragmentsource);
...
void renderGraphics2D(Graphics2D *g2D) {
	...
	glActiveTexture(g2D->texUnit);
	glBindTexture(GL_TEXTURE_2D, g2D->textureID);
	
	glDrawElements(GL_QUADS, g2D->numElements, GL_UNSIGNED_INT, 0);
